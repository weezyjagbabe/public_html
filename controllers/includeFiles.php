<?php
	...
	if ( $pageName === "Transaction" )
	{
		...
		if ( isset( $_POST['continue'] ) ) { $step = "2: Confirm the transaction details"; }
		elseif ( isset( $_POST['proceed'] ) || isset( $_POST['selectSource'] ) ) { $step = "3: Select source of fund and pay"; }
		else{ $step = "1: Complete the service form"; }
	}

    if ( $pageName === "Verified Transaction" )
    {
        if( isset( $_GET['t'] ) ) { $tKey = $_GET['t']; } else { redirect( 'services' ); } 					                    // If there is a serviceKey grab it else redirect to services page
        if( !$UserClass -> userIsLoggedin() ) { redirect( 'signin' ); }										                    // Redirect to signin page if user is not logged in
//        $validation -> validateParam( $sKey, "tlr_services", "serviceKey" );								    // Validate parameters and proceed
        require_once './models/classes/transactionsClass.php';												                        // Include the user transactions class file
...
//        else{ $step = "1: Complete the service form"; }
	...
	if ( $pageName === "Transaction Receipt" || $pageName === "Transaction Response" ) 
	{
		...
		if( !$UserClass -> userIsLoggedin() ) { redirect( 'sign-in' ); }	 					                                    // Redirect to signin page if user is not logged in
	}
    ...
    elseif ( $pageName === "Reset Password" )
    {
        if( $UserClass -> userIsLoggedin() === TRUE ) { redirect( 'services' ); }							                    // Check if user is already logged in and redirect
        if ( !isset( $_GET['userEmail'] ) ) { redirect( 'signin' ); } else { $userEmail = $_GET['userEmail']; }								                            // Check if there is no activation code and redirect
    }

	elseif ( $pageName === "Cancel Transaction" ) 
	{
		if( isset( $_GET['t'] ) ) $tKey = $_GET['t'];														                        // If there is a transactionKey value, grab it
		$validation -> validateParam( $tKey, "tlr_transactions", "transactionKey" );						    // Validate parameters and proceed
		require_once './models/classes/transactionsClass.php';												                        // Include the user transactions class file
		$transactions -> updateStatus( $tKey, 2 );															                // Update the transaction status
		if( !$UserClass -> userIsLoggedin() ) { $UserClass -> redirect( 'signin' ); } 						                        // Redirect to signin page if user is not logged in
	}
	
	elseif ( $pageName === "Transaction Details" ) 
	{
		if( isset( $_GET['t'] ) ) $tKey = $_GET['t'];														                        // If there is a transactionKey value, grab it
		$validation -> validateParam( $tKey, "tlr_transactions", "transactionKey" );						    // Validate parameters and proceed
		if( !$UserClass -> userIsLoggedin() ) { $UserClass -> redirect( 'signin' ); } 						                        // Redirect to signin page if user is not logged in
