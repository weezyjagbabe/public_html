<?php
	$transactionDesc = "Mobile Money Transfer => theteller";																					// Set the transaction description

	// Check if the user submitted the form and show the summary form
	if ( isset( $_POST['continue'] ) ) 
	{
		$recipientNetwork		=	$_POST['recipientNetwork'];
		$recipientNumber		=	$_POST['recipientNumber'];
		$amountPaying			=	$_POST['amountPaying'];
		
		// Check if the $recipientNetwork is empty
		if ( $validation -> checkRequired( $recipientNetwork ) ) 
		{
			// Echo the error and show the form again
			echo'<div class="alert alert-danger"><strong>Oh snap!</strong> Select recipient network.</div>';
			?>
				<form class="validate" action="" method="post" role="form" id="form1">
					<div class="form-group">
						<label class="control-label">Recipient Network</label>
						<select name="recipientNetwork" class="form-control" data-validate="required">
							<option value="<?php echo $recipientNetwork; ?>"><?php echo $recipientNetwork; ?></option>
							<?php
								$columns = Array ( "telcoName" );
								$DatabaseHandler -> where( 'telcoStatus', 1 );
								$telcos = $DatabaseHandler -> get ( "tlr_telcos", null, $columns );
								if ( $DatabaseHandler -> count > 0 )
								foreach ( $telcos as $telco ) 
								{ 
									?>
										<option value="<?php echo $telco['telcoName']; ?>"><?php echo $telco['telcoName']; ?></option>
									<?php
								}
							?>
						</select>
					...
					<div class="form-group">
						<label class="control-label">Recipient Number</label>
						<input type='text' class="form-control" name='recipientNumber' data-validate="required" value="<?php echo $recipientNumber; ?>" required="" autocomplete="off" />
					</div>	
		...
		// Check if the $recipientNumber is empty
		elseif ( $validation -> checkRequired( $recipientNumber ) ) 
		{
			// Echo the error and show the form again
			echo'<div class="alert alert-danger"><strong>Oh snap!</strong> Enter recipient number.</div>';
			?>
				<form class="validate" action="" method="post" role="form" id="form1">
					<div class="form-group">
						<label class="control-label">Recipient Network</label>
		...
		{
			// Echo the error and show the form again
			echo'<div class="alert alert-danger"><strong>Oh snap!</strong> Enter amount.</div>';
			?>
				<form class="validate" action="" method="post" role="form" id="form1">
					<div class="form-group">
						<label class="control-label">Recipient Network</label>
		...
		{
//			If( $tKey = $transactions -> addTransaction( $sKey, $recipientNumber, $amountPaying, $transactionDesc. " => " . $recipientNumber, $transactionExtraData = json_encode( array( 'recipientNetwork' => strtolower( $recipientNetwork ), 'recipientNumber' => $recipientNumber ) ) ) )
                If( $tKey = $transactions -> addTransaction( $sKey, $recipientNumber, $amountPaying, $transactionDesc. " => " . $recipientNumber, $transactionExtraData = $_POST )  )
			{
				...
				?>
					<form class="validate" action="" method="post" role="form" id="form1">
						<div class="form-group">
							<label class="control-label">Recipient Number</label>
							<input type='text' class="form-control" value="<?php echo $recipientNumber; ?>" autocomplete="off" readonly/>
						</div>
...
//            sendTransactionMail($_POST['tKey']);																		    // Send the receipt to user by email
    ...
    elseif ( isset( $_POST['payNow'] ) )
    {
        ...
        {
            $responseCode = $transactions -> processMoneyTransfer( $_POST['tKey'] );												    // Process the payment and return the response code
//            var_dump($responseCode);
	...
	else
	{
		?>
			<form class="validate" action="" method="post" role="form" id="form1">
				...
				<div class="form-group">
					<label class="control-label">Recipient Number</label>
					<input type='text' class="form-control" name='recipientNumber' data-validate="required" placeholder='Enter recipient phone number' required="" autocomplete="off" />
				</div>	
