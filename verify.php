<?php
$pageName = "Verify";																			// Set the page name
...
//var_dump($_GET);
    if($_GET['code'] == 000 )																					    // Check the response of the transaction
    {
        ...
        $transactions -> updateVerificationTransactionExtraData( $responseCode = json_encode( array( 'responseCode' => $_GET ) ), $tKey );
        $_SESSION['count'] = 1;
        $transactions -> updateVerificationStatus( $tKey, 1 );															    // Transaction is successfull, update the status
//        sendTransactionMail($tKey);																		    // Send the receipt to user by email
        echo '<script type="text/javascript" language="javascript">
                        window.top.location.href="http://45.56.99.27/verificationreceipt?t='.$tKey.'";
...
//        redirect( '../response?t='.$tKey );																	// Redirect the user to the response page
                        echo '<script type="text/javascript" language="javascript">
                        window.top.location.href="http://45.56.99.27/verificationresponse?t='.$tKey.'";
                       </script>';
    }
